...
from src.auth import create_access_token  # Ensure this import is correct
...
def test_login_user(client, unique_username, unique_email):
    username = create_user(client, unique_username, unique_email, "loginpassword")[
        "username"
    ]
    response = client.post(
        "/login", json={"username": username, "password": "loginpassword"}
    )
    assert (
        response.status_code == 200
    ), f"Response status code: {response.status_code}, Response body: {response.text}"
...
def test_reset_password(client, unique_username, unique_email):
    username, email, _ = create_user(
        client, unique_username, unique_email, "testpassword"
    ).values()
    token = login_user(client, username, "testpassword")
    headers = {"Authorization": f"Bearer {token}"}

    response = client.post(
        "/users/reset-password", params={"email": email}, headers=headers
    )
    assert (
        response.status_code == 200
    ), f"Response status code: {response.status_code}, Response body: {response.text}"
...
def test_user_deactivation(client, unique_username, unique_email):
    username = create_user(client, unique_username, unique_email, "deactivatepassword")[
        "username"
    ]
...
def test_token_refresh(client, unique_username, unique_email):
    username = create_user(client, unique_username, unique_email, "refreshpassword")[
        "username"
    ]
    login_response = client.post(
        "/login", json={"username": username, "password": "refreshpassword"}
    )
    assert (
...
def test_token_expiry(client, unique_username, unique_email):
    username = create_user(client, unique_username, unique_email, "expirypassword")[
        "username"
    ]
